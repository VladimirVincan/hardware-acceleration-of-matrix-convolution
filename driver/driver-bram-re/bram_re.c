...
MODULE_LICENSE("GPL");
MODULE_DESCRIPTION("Driver for BRAMRE");
#define DEVICE_NAME "bramRE"
#define DRIVER_NAME "bramRE_driver"

#define MATRIX_HEIGHT 8
#define MATRIX_WIDTH 8

//*************************************************************************
static int bramRE_probe(struct platform_device *pdev);
static int bramRE_open(struct inode *i, struct file *f);
static int bramRE_close(struct inode *i, struct file *f);
static ssize_t bramRE_read(struct file *f, char __user *buf, size_t len, loff_t *off);
static ssize_t bramRE_write(struct file *f, const char __user *buf, size_t count, loff_t *off);
static int __init bramRE_init(void);
static void __exit bramRE_exit(void);
static int bramRE_remove(struct platform_device *pdev);
...
//*********************GLOBAL VARIABLES*************************************
static struct file_operations bramRE_fops =
{
	.owner = THIS_MODULE,
	.open = bramRE_open,
	.release = bramRE_close,
	.read = bramRE_read,
	.write = bramRE_write
};
static struct of_device_id bramRE_of_match[] = {
	{ .compatible = "xlnx,bram_re", },     // TODO
	{ /* end of list */ },
};

static struct platform_driver bramRE_driver = {
	.driver = {
		.name = DRIVER_NAME,
		.owner = THIS_MODULE,
		.of_match_table	= bramRE_of_match,
	},
	.probe		= bramRE_probe,
	.remove	= bramRE_remove,
};
...
static struct bramRE_info *vp = NULL;

MODULE_DEVICE_TABLE(of, bramRE_of_match);
...
static int bramRE_probe(struct platform_device *pdev)
{
	...
	vp = (struct bramRE_info *) kmalloc(sizeof(struct bramRE_info), GFP_KERNEL);
	if (!vp) {
		printk(KERN_ALERT "Cound not allocate bramRE device\n");
		return -ENOMEM;
	}
	...
	if (!request_mem_region(vp->mem_start,vp->mem_end - vp->mem_start + 1, DRIVER_NAME))
	{
		...
		printk(KERN_INFO "bramRE_init: Successfully allocated memory region for bramRE\n");
	}
	/*
	...
	if (!vp->base_addr) {
		printk(KERN_ALERT "bramRE: Could not allocate iomem\n");
		rc = -EIO;
		goto error2;
	}
...
static int bramRE_open(struct inode *i, struct file *f)
{
	//printk("bramRE opened\n");
	return 0;
}
static int bramRE_close(struct inode *i, struct file *f)
{
	//printk("bramRE closed\n");
	return 0;
}
...
static int __init bramRE_init(void)
{
        ...
        printk(KERN_INFO "bramRE_init: Initialize Module \"%s\"\n", DEVICE_NAME);
        
        if (alloc_chrdev_region(&first, 0, 1, "BRAMRE_region") < 0)
        {       
        ...
        if (device_create(cl, NULL, MKDEV(MAJOR(first),0), NULL, "bramRE") == NULL)
        {       
                goto fail_1;
        }
        ...
        return platform_driver_register(&bramRE_driver);
...
static void __exit bramRE_exit(void)
{
	...
	platform_driver_unregister(&bramRE_driver);
	cdev_del(&c_dev);
	device_destroy(cl, MKDEV(MAJOR(first),0));
	class_destroy(cl);
	unregister_chrdev_region(first, 1);
	printk(KERN_INFO "bramRE_exit: Exit Device Module \"%s\".\n", DEVICE_NAME);
}

module_init(bramRE_init);
module_exit(bramRE_exit);

MODULE_AUTHOR ("FTN");
MODULE_DESCRIPTION("Test Driver for BRAMRE output.");
MODULE_LICENSE("GPL v2");
MODULE_ALIAS("custom:bramRE");
